...
const useDelete = (initialInvoices: InvoiceCardProps[]) => {
    ...
    const confirmDelete = () => {
        if (invoiceToDelete) {
            setInvoiceList(invoiceListDelte.filter(i => i.invoiceNumber !== invoiceToDelete.invoiceNumber));
    ...
    return { invoiceListDelte, handleDelete, confirmDelete, cancelDelete, showConfirmDelete, isConfirmed };
};
